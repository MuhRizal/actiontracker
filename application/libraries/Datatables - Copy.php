...
*/
    protected function get_filtering()
    {
	  ...
	  $link = mysqli_connect( $this->ci->db->hostname, $this->ci->db->username, $this->ci->db->password);
      $sSearch = mysqli_real_escape_string($link,$this->ci->input->get('sSearch'));
...
*/
    protected function get_display_result()
    {
		if(($this->custom_query)!=""){
			$tempQuery = $this->ci->db->get($this->custom_query);
		}else
			$tempQuery = $this->ci->db->get();		
		$this->active_query = $this->ci->db->last_query();
		return $tempQuery;
    }
...
/**
define custom query here
**/
	function set_custom_query($query){
		$this->custom_query = "(".$query.") mytable ";
		$this->table = "(".$query.") mytable ";
	}
